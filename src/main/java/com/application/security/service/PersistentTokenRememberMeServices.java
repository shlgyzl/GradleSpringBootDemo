...
import com.application.common.util.RandomUtil;
import com.application.web.domain.jpa.PersistentToken;
import com.application.web.repository.jpa.PersistentTokenRepository;
...
public class PersistentTokenRememberMeServices extends
        ...
        synchronized (this) { // prevent 2 authentication requests from the same user in parallel
            ...
            if (login == null) {
                ...
                try {
                    ...
                    throw new RuntimeException("Autologin failed due to data access problem", e);
                }
    ...
    protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication
        ...
        PersistentToken token = userRepository.findByLogin(login).map(u -> {
            PersistentToken t = new PersistentToken();
            t.setId(RandomUtil.generateSeriesData());
            t.setUser(u);
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (rememberMeCookie != null && rememberMeCookie.length() != 0) {
            try {
                ...
                PersistentToken token = getPersistentToken(cookieTokens);
                persistentTokenRepository.deleteById(token.getId());
